...
class RegistrationPage {
  get formRoot() { return $('.login-form'); }
  get enterAccountInfoHeader() { return this.formRoot.$('h2=Enter Account Information'); }
  get addressInfoHeader() { return this.formRoot.$('h2=Address Information'); }
  get titleMrLabel() { return $('label[for="id_gender1"]'); }
  get titleMrsLabel() { return $('label[for="id_gender2"]'); }
  ...
  get newsletterLabel() { return $('label[for="newsletter"]'); }
  get offersCheckbox() { return $('#optin'); }
  get offersLabel() { return $('label[for="optin"]'); }
  ...
  async setPreferenceToggles({ newsletter, offers }) {
    ...
    if (newsletter !== undefined && newsletter !== nl) await this.newsletterLabel.click();

    const of = await this.offersCheckbox.isSelected();
    if (offers !== undefined && offers !== of) await this.offersLabel.click();
  }

  async fillAddressInfo({ firstName, lastName, company, address, address2, country, state, city, zipcode, mobile }) {
